#!/bin/bash
# Deploy Db2 source connector (lands both IOT_DEVICES and IOT_DEVICES_MEASUREMENTS tables into Kafka)
# Pre-creates the compacted iot_devices_db2 topic before deploying the connector

set -e

CONNECT_HOST="${CONNECT_HOST:-localhost:8083}"
BROKER_HOST="${BROKER_HOST:-localhost:9092}"
...
# Wait until DB2 accepts JDBC connections AND both IOT_DEVICES tables exist
wait_for_db2() {
    echo "Waiting for DB2 + IOT_DEVICES tables (can take 3-5 min on first boot)..."
    local start
    start=$(date +%s)
    local attempt=0
    ...
    while [ $attempt -lt $max ]; do
        ...
        # Check both: DB2 accepts connections AND both tables exist
        if docker exec db2-luw su - db2inst1 \
               -c 'db2 connect to testdb > /dev/null 2>&1 &&
                   ...
                   db2 "SELECT COUNT(*) FROM DB2INST1.IOT_DEVICES_MEASUREMENTS" > /dev/null 2>&1 &&
                   db2 connect reset > /dev/null 2>&1' \
               2>/dev/null; then
            echo "DB2 ready and both IOT_DEVICES tables exist (${elapsed}s)."
            return 0
        fi

        printf "  [%3ds] attempt %d/%d — waiting for DB2 and tables...\n" \
               "$elapsed" "$attempt" "$max"
        sleep 10
    done
    echo "ERROR: DB2/tables did not become ready within $(( max * 10 ))s." >&2
    exit 1
}

create_compacted_topic() {
    echo "Pre-creating compacted topic 'iot_devices_db2'..."
    # Check if topic already exists
    if docker exec broker kafka-topics --bootstrap-server localhost:9092 --list 2>/dev/null | grep -q "^iot_devices_db2$"; then
        echo "  Topic 'iot_devices_db2' already exists."
        return 0
    fi
    ...
    echo "  Topic created with cleanup.policy=compact."
}
...
echo ""
echo "Checking and recovering any FAILED connector..."
